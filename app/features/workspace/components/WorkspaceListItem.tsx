import React from 'react';
import type { SidebarWorkspaceEntry } from '@/components/ui/Sidebar';
import { ChevronRight } from 'lucide-react';
import { getWorkspaceCopy } from '../copy';

interface WorkspaceListItemProps {
  workspace: SidebarWorkspaceEntry;
  onClick: () => void;
}

export function WorkspaceListItem({ workspace, onClick }: WorkspaceListItemProps) {
  const isOk = workspace.status === 'ok';
  const copy = getWorkspaceCopy();

  return (
    <div
      onClick={onClick}
      className={`flex items-center px-4 py-3 cursor-pointer bg-surface-container hover:bg-surface-container-high border border-on-surface/10 hover:border-primary/30 rounded-xl transition-all group ${
        !isOk ? 'opacity-70' : ''
      }`}
    >
      <div className="w-8 h-8 rounded-lg bg-surface flex items-center justify-center text-primary font-manrope font-bold text-sm mr-4 shadow-sm">
        {workspace.name.charAt(0).toUpperCase()}
      </div>
      
      <div className="flex-1 min-w-0 flex items-center gap-4">
        <h3 className="font-manrope font-semibold text-sm text-on-surface group-hover:text-primary transition-colors truncate">
          {workspace.name}
        </h3>
        <p className="font-inter text-xs text-on-surface-variant truncate hidden sm:block">
          {workspace.rootPath}
        </p>
      </div>

      <div className="flex items-center gap-3">
        {!isOk && (
          <span className="text-[10px] uppercase font-bold text-red-600 bg-red-100 px-2 py-0.5 rounded-md">{copy.sidebar.errorBadge}</span>
        )}
        <ChevronRight size={16} className="text-on-surface-variant opacity-0 group-hover:opacity-100 transition-opacity -translate-x-2 group-hover:translate-x-0" />
      </div>
    </div>
  );
}
