...
export class BidirectionalCompactStrategy implements LayoutStrategy {
    async layoutGroup(context: LayoutContext): Promise<Map<string, { x: number; y: number }>> {
        ...
        const leftPos = runFlextreeLayout(leftNodes, leftEdges, spacing);
        const rightPos = runFlextreeLayout(rightNodes, rightEdges, spacing);
        ...
        // LEFT side: invert x to go leftward from root
