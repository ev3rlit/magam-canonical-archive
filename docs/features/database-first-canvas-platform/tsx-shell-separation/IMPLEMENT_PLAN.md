...
3. 남아 있는 chat 모듈은 product shell이 아닌 CLI 내부 보조 boundary로 격리한다.

구현 결과:

- `app/features/host/rpc/*` 의 chat method surface 제거 완료
- `libs/cli/src/server/http.ts` 의 chat endpoints 제거 완료
- editor shell의 chat panel entrypoint 제거 완료
- app router `app/app/api/chat/*` 삭제 완료
...
- canonical document shell은 shared lower layer(`libs/shared/src/lib/canonical-document-shell/*`)로 고정했고, route/backend는 해당 shell만 재사용한다.
- renderer는 문서 생성과 reopen 흐름에서 canonical document metadata를 app-state/session 복원 경계까지 유지한다.
- TSX materialization은 compatibility source 생성 helper로만 남기고, desktop `/render`의 canonical-document fallback도 그 helper를 사용한다.
- product shell chat 제거는 renderer page, RPC contract, web API routes, desktop HTTP backend, 관련 테스트까지 함께 반영해 parity drift를 막았다.
