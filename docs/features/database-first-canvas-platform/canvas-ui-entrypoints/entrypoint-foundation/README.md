...
핵심 목표는 이후 `canvas-toolbar`, `selection-floating-menu`, `pane-context-menu`, `node-context-menu`가 같은 shell 파일을 두고 충돌하지 않도록, shared host와 contract를 먼저 잠그는 것이다.
...
- 이후 `canvas-toolbar`, `selection-floating-menu`, `pane-context-menu`, `node-context-menu`는 이 4개 foundation slice를 조합해서 소비한다.

## 완료 기준

- 이후 4개 UI surface가 shared host와 resolver를 재사용할 수 있다.
- surface별 작업이 같은 core 파일 충돌 없이 진행 가능하다.
