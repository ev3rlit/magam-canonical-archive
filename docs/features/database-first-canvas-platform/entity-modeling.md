...
- `props`, `style`, `tailwind utility`, `binding`, `layout`을 어디에 둘 것인가
...
- `props`, `style`, `layout`, `viewport_state`, `binding_config`는 단순 JSON blob가 아니라 타입이 있는 값 객체다.
...
4. **Value Object**
   - `layout`, `style`, `props`, `binding`처럼 identity 없이 값으로 비교되는 타입
...
3. plugin source와 plugin instance는 분리한다.
4. `props`, `style`, `tailwind`, `binding`은 top-level entity가 아니라 typed value object로 다룬다.
5. Drizzle schema는 persistence contract이고, 애플리케이션 entity는 그 위의 해석 계층이다.
...
### ObjectGraphAggregate

- 루트: `ObjectEntity`
- 포함: `ObjectRelationEntity`, `EmbeddingRecordEntity`
- 책임: 의미 데이터와 graph relation
...
## 5.2 Canonical Object Graph

### ObjectEntity

용도:

- 문서가 참조할 수 있는 의미 데이터의 canonical entity
- 여러 문서가 같은 object를 다른 위치/스타일로 재사용 가능
...
- 의미 데이터는 직접 소유하지 않고, 필요하면 `canonicalObjectId` 또는 `CanvasBindingEntity`를 통해 연결한다.
...
- node type별 기능 props
- 예: shape variant, text align, chart options, note mode, plugin widget input
...
- `props`는 node/component의 기능 입력값이어야 한다.
...
| `ObjectEntity` | `objects` | workspace-scoped canonical object |
...
- `objectRepository.loadWorkspaceObjectGraph(workspaceId)`
