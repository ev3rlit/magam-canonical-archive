...
- primary renderer/RPC/backend shell에서는 chat surface를 제거했고, 남은 chat 구현은 CLI 내부 보조 모듈로만 남는다.
...
- canonical document list/create/open ownership은 `libs/shared/src/lib/canonical-document-shell/*`로 고정했다.
- web `app/app/api/documents/route.ts`와 desktop `libs/cli/src/server/http.ts`는 같은 canonical document shell을 재사용한다.
- renderer workspace/document flow는 canonical metadata(`documentId`, `workspaceId`, `latestRevision`)를 유지하고, 문서 오픈은 canonical document registry를 먼저 통과한다.
- TSX는 compatibility materialization과 legacy tree/watch 경로에만 남기고, `libs/shared/src/lib/workspace-shell.ts`와 `libs/cli/src/server/http.ts`에서는 compatibility 명명으로만 노출한다.
- primary shell chat surface는 `CanvasEditorPage`, renderer RPC contracts/adapters, Next `/api/chat/*`, desktop HTTP `/chat/*`에서 제거했다.
