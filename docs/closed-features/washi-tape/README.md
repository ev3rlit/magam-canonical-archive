...
1. 사용자가 `<WashiTape pattern={preset('pastel-dots')}>TODO</WashiTape>`로 빠르게 강조 라벨을 만든다.
...
// 프리셋 ID 유니온 타입 (PRESET_NAMES에서 파생)
type MaterialPresetId = (typeof PRESET_NAMES)[number];
...
// 소재 입력 전체 유니온
type PaperMaterial = PresetPattern | SVGPattern | SolidPattern | ImagePattern;
...
// 문자열이 MaterialPresetId 유효값인지 판별하는 타입가드
function isMaterialPresetId(value: string): value is MaterialPresetId {
  return (PRESET_NAMES as readonly string[]).includes(value);
}
```

### Helper API (`@magam/core`)

```ts
preset(id: MaterialPresetId): PaperMaterial
svg(opts: { src?: string; markup?: string }): PaperMaterial
solid(color: string): PaperMaterial
image(src: string, opts?: { scale?: number; repeat?: 'repeat-x' | 'repeat' | 'stretch' }): PaperMaterial
definePattern(def: PaperMaterial): PaperMaterial
...
1. 패턴 우선순위: `pattern` > default preset
...
1. 타입/헬퍼 정의 (`PaperMaterial`, `AtDef`, `EdgeDef`, `TextureDef`, `TextDef`)
